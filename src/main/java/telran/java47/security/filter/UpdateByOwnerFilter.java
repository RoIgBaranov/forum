...
public class UpdateByOwnerFilter implements Filter {
	...
	private boolean checkEndPoint(HttpMethod method, String path) {
		return HttpMethod.PUT.equals(method) && path.matches("/account/user/\\w+/?") 
				|| HttpMethod.POST.equals(method) && path.matches("/forum/post/\\w+/?")
				|| HttpMethod.PUT.equals(method) && path.matches("/forum/post/\\w+/comment/\\w+/?");
	}
